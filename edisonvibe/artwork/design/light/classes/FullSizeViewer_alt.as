...
class FullSizeViewer extends MovieClip {
	...
	public function loadImage(_image:String):Void {
		...
		// --------------
		this.createEmptyMovieClip("sIconHolder_mc", 3);
		this.attachMovie("MC MovieClipLoaderUI", "preloader_mc", 4);
		centerClip(preloader_mc); // position of loaded full image
		preloader_mc.init(_image, sIconHolder_mc, this, imageLoaded); 
	}
	//----------------------------------------------------------------------------------
	...
	private function centerClip(_clip:MovieClip):Void {
		_clip._x = 17//(bg_mc._width-_clip._width)*.5;
		_clip._y = 68//(bg_mc._height-_clip._height)*.5;
		_clip._rotation = -5
		//_clip._xscale =85;
		//_clip._yscale =85;		
	}
